...
Strategy.prototype.authenticate = function(req, options) {
  ...
  if (req.query && req.query.token_type && req.query.token_type == "Bearer"){
    var token = {
    ...
    var idToken = token.id_token;
      ...
      jwt.decode(self._clientSecret, idToken, function (err_, decode) {
      if (err_) {
        return self.error(new Error(err_));
      } else {
        console.log("JWT:" + JSON.stringify(decode, null, 4));
        ...
        profile.id = decode.sub;
        // Prior to OpenID Connect Basic Client Profile 1.0 - draft 22, the
        // "sub" key was named "user_id".  Many providers still use the old
        // key, so fallback to that.
        if (!profile.id) {
          profile.id = decode.user_id;
        }
        
        profile.displayName = decode.email;
        profile.name = { familyName: decode.family_name,
                          givenName: decode.given_name,
                          middleName: decode.middle_name };
        
        // profile._raw = body;
        profile._json = decode;
      ...
      if (arity == 8) {
        self._verify(iss, sub, profile, /*jwtClaims*/{}, token.accessToken, {refreshToken : token.refreshToken, expiresIn : token.expiresIn}, params, verified);
      } else if (arity == 7) {
        self._verify(iss, sub, profile, token.accessToken, {refreshToken : token.refreshToken, expiresIn : token.expiresIn}, params, verified);
      } else if (arity == 6) {
        self._verify(iss, sub, profile, token.accessToken, {refreshToken : token.refreshToken, expiresIn : token.expiresIn}, verified);
      } else if (arity == 4) {
    ...
    console.log("Redirect URI: " + location);
